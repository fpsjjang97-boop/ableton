...
if (!$boardDB) {
    header('Location: board_list.php?board=free');
    exit;
}

// 시각 정보: 하드코딩에 있으면 사용, 없으면 DB의 color_class/bg_class로 폴백
if (isset($boardVisual[$currentBoard])) {
    ...
    $boardVis = [
        'write_title' => $boardDB['write_title'] ?: '글쓰기',
        'icon' => $iconSvg,
...
<style>
    ...
    .note-toolbar .note-color-btn .note-recent-color {
        border-radius: 2px;
    }
    .note-editing-area {
        background: #141414 !important;
    ...
    .note-editing-area .note-editable[data-placeholder]::before {
        color: rgba(113,113,122,0.4) !important;
    }
    .note-statusbar {
        background: #18181b !important;
    ...
    <section class="border-b border-suno-border">
        <div class="max-w-3xl mx-auto px-6 py-8">
            <div class="flex items-center gap-3">
                <a href="board_list.php?board=<?php echo $currentBoard; ?>" class="w-9 h-9 flex items-center justify-center rounded-lg border border-suno-border text-suno-muted hover:text-white hover:border-suno-accent/40 transition-all">
                ...
                <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 <?php echo $board['bg']; ?> border rounded-lg flex items-center justify-center">
                        ...
                        <svg class="w-4.5 h-4.5 <?php echo $board['color']; ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <?php echo $board['icon']; ?>
                        </svg>
                        <?php else: ?>
    ...
    <section class="py-8">
        <div class="max-w-3xl mx-auto px-6">
            <form action="board_write_ok.php" method="POST">
                ...
                <input type="hidden" name="category_id" id="category_id" value="<?php echo !empty($categories) ? $categories[0]['id'] : ''; ?>">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <button type="button" class="cat-chip text-xs border border-suno-border bg-suno-surface rounded-lg px-3.5 py-2 text-suno-muted <?php echo $i === 0 ? 'selected' : ''; ?>" data-cat-id="<?php echo $cat['id']; ?>" onclick="selectCategory(this)">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center gap-3">
                        <input type="number" name="recruit_count" min="1" max="20" value="1"
                ...
                <div class="mb-6">
                    ...
                    <div class="flex gap-2">
                        ...
                        <input type="text" name="contact_info" id="contactInfo" placeholder="오픈채팅 링크를 입력하세요"
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-suno-border">
                    <a href="board_list.php?board=<?php echo $currentBoard; ?>" class="cancel-btn px-6 py-3 border border-suno-border rounded-xl text-sm text-suno-muted font-medium">
                        취소
                    </a>
                    <button type="submit" class="submit-btn px-8 py-3 rounded-xl text-sm text-white font-semibold">
                        <?php echo htmlspecialchars($board['write_title']); ?>
                    </button>
                </div>
            </form>
        </div>
